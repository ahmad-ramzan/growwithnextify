...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-xl border-b border-gray-100 shadow-[0_4px_20px_rgba(30,93,153,0.05)]" : "bg-transparent py-2"}`}>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-6 py-6 flex flex-col gap-4">
          ...
          <a href="#" onClick={() => setOpen(false)} className="btn-primary mt-4 justify-center">Book a Call</a>
        </div>
      )}
    </header>
  );
